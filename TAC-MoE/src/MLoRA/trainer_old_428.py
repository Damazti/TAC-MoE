...
# is_fairscale_available was removed in transformers>=4.42; fairscale is no longer supported
...
# These version checks were removed in transformers>=4.42; we use PyTorch 2.x so always True/False
...
class Trainer:
    ...
    def _set_signature_columns_if_needed(self):
        if self._signature_columns is None:
            ...
            self._signature_columns += list(set(["label", "label_ids", "task_id", "depart", "entity"] + self.label_names))
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_length:
            ...
            if self.args.world_size <= 1:
                if self.args.batched_training:
                    from utils.sampler import BatchedTaskSampler
                    return BatchedTaskSampler(self.train_dataset, self._train_batch_size)
                else:
                ...
                if self.args.batched_training:
                    from utils.sampler import DistributedBatchedTaskSampler, UniformDistributedBatchedTaskSampler, NewDistributedBatchedTaskSampler
                    return NewDistributedBatchedTaskSampler(
    ...
    def get_test_dataloader(self, test_dataset: Dataset) -> DataLoader:
        ...
        if is_datasets_available() and isinstance(test_dataset, datasets.Dataset):
            test_dataset = self._remove_unused_columns(test_dataset, description="test")    # only remain the necessary columns
        else:
    ...
    def _inner_training_loop(
        ...
        if args.deepspeed:
            deepspeed_engine, optimizer, lr_scheduler = deepspeed_init(
                self, num_training_steps=max_steps, resume_from_checkpoint=None # Resume_training, the original is resume_from _checkpoint
            )
            self.model = deepspeed_engine.module
            self.model_wrapped = deepspeed_engine
            self.deepspeed = deepspeed_engine
            self.optimizer = optimizer  # TODO: only remove this line and add self.create_optimizer_and_scheduler(num_training_steps=max_steps)
        ...
        # Free disk: delete DeepSpeed state files after loading into GPU memory
        if resume_from_checkpoint is not None and args.deepspeed:
            import glob as _glob
            ds_dirs = _glob.glob(os.path.join(resume_from_checkpoint, "global_step*"))
            for ds_dir in ds_dirs:
                if os.path.isdir(ds_dir):
                    import shutil
                    shutil.rmtree(ds_dir, ignore_errors=True)
                    logger.warning(f"Deleted loaded DeepSpeed state to free disk: {ds_dir}")
        ...
        if resume_from_checkpoint is not None:
            self.state.max_steps += self.state.global_step  # Resume_training
        ...
        epochs_trained = 0  # Resume_training
        _interrupted = False
        try:
          for epoch in range(epochs_trained, num_train_epochs):
            ...
            if args.load_best_model_at_end and self.state.best_model_checkpoint is not None:
                logger.warning(f"Loading best model from {self.state.best_model_checkpoint} (best metric: {self.state.best_metric})")
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        self.save_model(output_dir, _internal_call=True)
        if self.deepspeed:
            # Skip DeepSpeed optimizer state (~24GB) to avoid filling disk
            # Adapter weights are already saved by save_model() on line 2350 above
            logger.info(f"Step {self.state.global_step}: adapter weights saved, skipping DeepSpeed optimizer state to save disk")
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # --- Achievement-based multi-task loss ---
        if task_ids is not None and self.achievement_loss is not None:
            # Keep labels in inputs so the model forward works normally
            ...
            if logits is not None and labels is not None:
                ...
                # Compute per-sample CE loss (causal LM: shift by 1)
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        if self.tokenizer is not None:  # save the tokenizer
            self.tokenizer.save_pretrained(output_dir)

        # Good practice: save your training arguments together with the trained model
        torch.save(self.args, os.path.join(output_dir, TRAINING_ARGS_NAME)) # save the training args
