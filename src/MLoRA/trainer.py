...
class Trainer:
    ...
    def __init__(
        ...
        # Achievement-based multi-task loss (set externally by main)
        self.achievement_loss = None
        self.achievement_warmup_ratio = 0.0
        self.achievement_scores_ready = False
        # Routing divergence loss (set externally by main)
        self.routing_div_coeff = 0.0
        self.routing_div_frozen_tasks = set()  # task IDs to diverge from
        self.lb_loss_coeff = 0.0  # LB loss coefficient (set externally)
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        # Use task-balanced sampler if task IDs were injected
        _task_ids = getattr(self, '_train_task_ids', None)
        if _task_ids is not None:
            from src.MLoRA.task_balanced_sampler import TaskBalancedSampler
            _max_os = getattr(self, '_max_oversample', 0.0)
            logger.info(f"Using TaskBalancedSampler (max_oversample={_max_os})")
            return TaskBalancedSampler(
                task_ids=_task_ids,
                batch_size=self._train_batch_size,
                max_oversample=_max_os,
                seed=self.args.seed,
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            # Propagate epoch to underlying sampler (DataLoaderShard lacks set_epoch,
            # so the above branch is skipped — reach through to the actual sampler)
            _underlying_sampler = getattr(
                getattr(train_dataloader, "batch_sampler", None),
                "batch_sampler", None,           # BatchSamplerShard wraps original
            )
            if _underlying_sampler is None:
                _underlying_sampler = getattr(train_dataloader, "batch_sampler", None)
            if _underlying_sampler is not None:
                _inner_sampler = getattr(_underlying_sampler, "sampler", None)
                if hasattr(_inner_sampler, "set_epoch"):
                    _inner_sampler.set_epoch(epoch)
                    logger.info(f"TaskBalancedSampler.set_epoch({epoch})")
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            # Skip grad_norm (always 0 with DeepSpeed ZeRO) and learning_rate to reduce noise
    ...
    def log(self, logs: Dict[str, float]) -> None:
        ...
        Only key metrics (loss, macro_f1_avg, accuracy, epoch) are printed
        to console.  All metrics are still recorded to log_history.
        """
        ...
        # Record ALL metrics to log_history (for later analysis)
        output = {**logs, **{"step": self.state.global_step}}
        self.state.log_history.append(output)

        # Filter: only print key metrics to console / TensorBoard
        # Include all per-dataset eval metrics (eval_*_f1, eval_*_accuracy, etc.)
        _ALWAYS_KEYS = {"loss", "epoch", "eval_loss", "eval_accuracy",
                        "eval_macro_f1_avg", "eval_binary_f1_avg"}
        _EVAL_SUFFIXES = ("_f1", "_accuracy", "_precision", "_recall")
        filtered = {}
        for k, v in logs.items():
            if k in _ALWAYS_KEYS:
                filtered[k] = v
            elif k.startswith("eval_") and any(k.endswith(s) for s in _EVAL_SUFFIXES):
                filtered[k] = v
        if filtered:
            self.control = self.callback_handler.on_log(self.args, self.state, self.control, filtered)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            result = self.compute_loss(model, inputs)
            # compute_loss returns (loss, con_loss) when con_loss is present, or just loss
            if isinstance(result, tuple):
                loss, con_loss = result
            else:
                loss = result
                con_loss = None
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # --- Multi-task loss path (handles per_sample_loss from chunked CE) ---
        if task_ids is not None:
            ...
            # Check if model returned per_sample_loss (memory-efficient chunked path)
            per_sample_loss = getattr(outputs, 'per_sample_loss', None)
            achievement_present = self.achievement_loss is not None
            achievement_active = achievement_present
            warmup_ratio = 0.0
            warmup_steps = 0
            if achievement_active:
                ...
                _debug_force_warmup_steps = __import__("os").environ.get("MOELORA_DEBUG_FORCE_WARMUP_STEPS")
                ...
                elif warmup_steps > 0 and not getattr(self, "achievement_scores_ready", False):
                    achievement_active = False

            if per_sample_loss is not None and labels is not None:
                if achievement_active:
                    loss = self.achievement_loss.compute_weighted_loss(per_sample_loss, task_ids)
                else:
                    # No achievement: simple mean of per_sample_loss
                    loss = per_sample_loss.mean()
            else:
                # Fallback: model returned full logits (e.g., ChatGLM3)
                ...
                if logits is not None and labels is not None:
                    ...
                    loss_fct = torch.nn.CrossEntropyLoss(
                        ...
                        **({"label_smoothing": ls} if ls > 0 else {}),
                    )
                    ...
                    valid_tokens = (shift_labels != -100).sum(dim=-1).clamp(min=1).float()
                    per_sample_loss = per_token_loss.sum(dim=-1) / valid_tokens
            ...
            # LB loss: controlled by lb_loss_coeff (0=disabled, 0.001=light regularization)
            lb_loss = getattr(outputs, 'lb_loss', None)
            _lb_coeff = getattr(self, 'lb_loss_coeff', 0.0)
            if lb_loss is not None and _lb_coeff > 0:
                loss = loss + _lb_coeff * lb_loss

            _debug_loss_steps = int(__import__("os").environ.get("MOELORA_DEBUG_LOSS_STEPS", "0") or 0)
            if _debug_loss_steps > 0 and self.state.global_step < _debug_loss_steps and self.args.local_rank <= 0:
                import json as _json_dbg
                import os as _os_dbg
                import torch as _torch_dbg
                with _torch_dbg.no_grad():
                    _payload = {
                        "step": int(self.state.global_step),
                        "achievement_present": bool(achievement_present),
                        "achievement_active": bool(achievement_active),
                        "warmup_ratio": float(warmup_ratio),
                        "warmup_steps": int(warmup_steps),
                        "loss": float(loss.detach().float().cpu()),
                    }
                    if task_ids is not None:
                        _tid_cpu = task_ids.detach().cpu() if hasattr(task_ids, "detach") else _torch_dbg.as_tensor(task_ids)
                        _uniq, _cnt = _torch_dbg.unique(_tid_cpu, return_counts=True)
                        _payload["task_counts"] = {str(int(k)): int(v) for k, v in zip(_uniq, _cnt)}
                        _payload["task_ids"] = [int(x) for x in _tid_cpu.tolist()]
                    if per_sample_loss is not None:
                        _ps = per_sample_loss.detach().float().cpu()
                        _payload["per_sample_mean"] = float(_ps.mean())
                        _payload["per_sample_std"] = float(_ps.std(unbiased=False)) if _ps.numel() > 1 else 0.0
                        if task_ids is not None:
                            _payload["per_task_loss_mean"] = {}
                            for _tid in _torch_dbg.unique(_tid_cpu):
                                _mask = (_tid_cpu == _tid)
                                _payload["per_task_loss_mean"][str(int(_tid))] = float(_ps[_mask].mean())
                    if labels is not None and hasattr(labels, "detach"):
                        _valid_labels = labels.detach()
                        _valid_labels = _valid_labels[_valid_labels != -100]
                        _payload["label_token_sum"] = int(_valid_labels.long().sum().cpu()) if _valid_labels.numel() else 0
                    if "input_ids" in inputs and hasattr(inputs["input_ids"], "detach"):
                        _ids = inputs["input_ids"].detach()
                        _payload["input_ids_sum"] = int(_ids.long().sum().cpu())
                        _payload["first_input_head"] = [int(x) for x in _ids[0, :12].detach().cpu().tolist()]
                    if self.achievement_loss is not None:
                        _payload["achievement_weights"] = {
                            str(k): float(v) for k, v in self.achievement_loss.weights.items()
                        }
                        _payload["achievement_scores"] = {
                            str(k): float(v) for k, v in self.achievement_loss.task_scores.items()
                        }
                _line = "[MOELORA_DEBUG_LOSS] " + _json_dbg.dumps(_payload, sort_keys=True)
                print(_line, flush=True)
                _debug_path = _os_dbg.path.join(self.args.output_dir, "debug_loss_path.jsonl")
                with open(_debug_path, "a") as _debug_f:
                    _debug_f.write(_json_dbg.dumps(_payload, sort_keys=True) + "\n")

            # Routing divergence loss: push new task's routing away from frozen tasks
            _div_coeff = getattr(self, 'routing_div_coeff', 0.0)
            _div_frozen = getattr(self, 'routing_div_frozen_tasks', set())
            if _div_coeff > 0 and _div_frozen and task_ids is not None:
                import torch as _t_div
                _unwrapped_div = self.accelerator.unwrap_model(model)
                _adapter_div = getattr(_unwrapped_div, 'active_adapter', 'default')
                _gate_div = None
                _te_div = None
                if hasattr(_unwrapped_div, 'lora_gate') and _adapter_div in _unwrapped_div.lora_gate:
                    _gate_div = _unwrapped_div.lora_gate[_adapter_div]
                if hasattr(_unwrapped_div, 'lora_task_embedding') and _adapter_div in _unwrapped_div.lora_task_embedding:
                    _te_div = _unwrapped_div.lora_task_embedding[_adapter_div]

                if _gate_div is not None and _te_div is not None:
                    # Find new task IDs (not in frozen set)
                    _new_tids = set(task_ids.unique().tolist()) - _div_frozen
                    if _new_tids:
                        # Compute frozen tasks' mean routing (detached — no gradient to frozen embeddings)
                        _frozen_routings = []
                        for _ft in _div_frozen:
                            if _ft < _te_div.weight.shape[0]:
                                _ft_em = _te_div.weight[_ft].detach().unsqueeze(0)
                                _ft_logits = _t_div.nn.functional.linear(_ft_em, _gate_div.GateL.weight.detach())
                                _ft_routing = _t_div.softmax(_ft_logits, dim=1)
                                _frozen_routings.append(_ft_routing)
                        if _frozen_routings:
                            _frozen_mean = _t_div.cat(_frozen_routings, dim=0).mean(dim=0, keepdim=True)  # (1, expert_num)

                            # Compute new task routing (WITH gradient to embedding)
                            _div_loss = _t_div.tensor(0.0, device=loss.device)
                            for _nt in _new_tids:
                                if _nt < _te_div.weight.shape[0]:
                                    _nt_em = _te_div.weight[_nt].unsqueeze(0)
                                    _nt_logits = _t_div.nn.functional.linear(_nt_em, _gate_div.GateL.weight.detach())
                                    _nt_routing = _t_div.softmax(_nt_logits, dim=1)  # (1, expert_num)
                                    # Penalize cosine similarity (maximize divergence)
                                    _cos = _t_div.nn.functional.cosine_similarity(
                                        _nt_routing, _frozen_mean.detach(), dim=1
                                    )
                                    _div_loss = _div_loss + _cos.mean()

                            loss = loss + _div_coeff * _div_loss

            # ---- Diagnostic logging (every logging_steps) ----
            _should_log = (
                self.state.global_step > 0
                and self.state.global_step % self.args.logging_steps == 0
                and self.args.local_rank <= 0  # only rank 0
            )
            if _should_log:
                import torch as _t
                _diag = {}

                # 1) Per-task mean loss (unweighted)
                if per_sample_loss is not None and task_ids is not None:
                    for _tid in task_ids.unique():
                        _mask = (task_ids == _tid)
                        _diag[f"train/task{_tid.item()}_loss"] = per_sample_loss[_mask].mean().item()

                # 2) LB loss
                if lb_loss is not None:
                    _diag["train/lb_loss"] = (lb_loss.item() if isinstance(lb_loss, _t.Tensor)
                                              else float(lb_loss))

                # 2b) Routing divergence loss
                if _div_coeff > 0 and '_div_loss' in dir():
                    _diag["train/routing_div_loss"] = (_div_loss.item() if isinstance(_div_loss, _t.Tensor)
                                                       else float(_div_loss))

                # 3) Expert routing distribution per task (from shared gate)
                _unwrapped = self.accelerator.unwrap_model(model)
                _gate = None
                _adapter = getattr(_unwrapped, 'active_adapter', 'default')
                if hasattr(_unwrapped, 'lora_gate') and _adapter in _unwrapped.lora_gate:
                    _gate = _unwrapped.lora_gate[_adapter]
                if _gate is not None and hasattr(_gate, '_last_gate_probs') and _gate._last_gate_probs is not None:
                    _probs = _gate._last_gate_probs.detach()  # (batch, expert_num)
                    _tids = _gate._last_task_ids              # (batch,) or None

                    if _tids is not None:
                        for _tid in _tids.unique():
                            _mask = (_tids == _tid)
                            _task_probs = _probs[_mask]             # (n, expert_num)
                            _mean_p = _task_probs.mean(dim=0)       # (expert_num,)
                            for _ei in range(_mean_p.shape[0]):
                                _diag[f"routing/task{_tid.item()}_expert{_ei}"] = _mean_p[_ei].item()
                            # Routing entropy: higher = more uniform
                            _ent = -(_mean_p * _t.log(_mean_p + 1e-8)).sum().item()
                            _diag[f"routing/task{_tid.item()}_entropy"] = _ent
                            # Max expert prob (specialization indicator)
                            _diag[f"routing/task{_tid.item()}_max_prob"] = _mean_p.max().item()
                    else:
                        _mean_p = _probs.mean(dim=0)
                        for _ei in range(_mean_p.shape[0]):
                            _diag[f"routing/global_expert{_ei}"] = _mean_p[_ei].item()

                    # 4) Routing divergence between tasks (key metric!)
                    if _tids is not None and len(_tids.unique()) == 2:
                        _tlist = sorted(_tids.unique().tolist())
                        _p1 = _probs[_tids == _tlist[0]].mean(dim=0)
                        _p2 = _probs[_tids == _tlist[1]].mean(dim=0)
                        # JS divergence (symmetric KL)
                        _m = 0.5 * (_p1 + _p2)
                        _kl1 = (_p1 * _t.log((_p1 + 1e-8) / (_m + 1e-8))).sum().item()
                        _kl2 = (_p2 * _t.log((_p2 + 1e-8) / (_m + 1e-8))).sum().item()
                        _diag["routing/task_divergence_JS"] = 0.5 * (_kl1 + _kl2)

                # 5) Achievement weights
                if self.achievement_loss is not None:
                    for _ak, _av in self.achievement_loss.weights.items():
                        _diag[f"achievement/weight_task{_ak}"] = _av
                    for _ak, _av in self.achievement_loss.task_scores.items():
                        _diag[f"achievement/ema_score_task{_ak}"] = _av

                # 6) Task embedding similarity (can the gate differentiate tasks?)
                _te = None
                if hasattr(_unwrapped, 'lora_task_embedding') and _adapter in _unwrapped.lora_task_embedding:
                    _te = _unwrapped.lora_task_embedding[_adapter]
                if _te is not None and _te.weight.size(0) >= 3:
                    _emb1 = _te.weight[1].detach().float()  # IHD
                    _emb2 = _te.weight[2].detach().float()  # Sarca
                    _cos = _t.nn.functional.cosine_similarity(_emb1, _emb2, dim=0).item()
                    _diag["gate/task_embedding_cosine_sim"] = _cos
                    _diag["gate/task_embedding_norm_IHD"] = _emb1.norm().item()
                    _diag["gate/task_embedding_norm_Sarca"] = _emb2.norm().item()

                # 7) Gate logits spread (pre-softmax, measures if gate WANTS to differentiate)
                if _gate is not None and _te is not None and _te.weight.size(0) >= 3:
                    _gw = _gate.GateL.weight.detach().float()  # (expert_num, te_dim)
                    for _tid_i, _tid_name in [(1, "IHD"), (2, "Sarca")]:
                        _logits_i = _gw @ _te.weight[_tid_i].detach().float()
                        _diag[f"gate/logit_spread_{_tid_name}"] = (_logits_i.max() - _logits_i.min()).item()
                        _diag[f"gate/logit_std_{_tid_name}"] = _logits_i.std().item()
                    # Gate weight matrix norm
                    _diag["gate/weight_frobenius_norm"] = _gw.norm().item()

                # 8) Per-task loss variance (gradient noise indicator)
                if per_sample_loss is not None and task_ids is not None:
                    for _tid in task_ids.unique():
                        _mask = (task_ids == _tid)
                        _tl = per_sample_loss[_mask]
                        if _tl.numel() > 1:
                            _diag[f"train/task{_tid.item()}_loss_std"] = _tl.std().item()

                if _diag:
                    # Write to diagnostics.jsonl (not printed, not in TensorBoard)
                    import json as _json, os as _os
                    _diag["step"] = self.state.global_step
                    _diag["epoch"] = self.state.epoch
                    _diag_path = _os.path.join(self.args.output_dir, "diagnostics.jsonl")
        ...
        # con_loss is optional (only present in TaCoMoE / contrastive models)
        con_loss = None
        if isinstance(outputs, dict) and "con_loss" in outputs:
            con_loss = outputs["con_loss"]

        if return_outputs:
            return (loss, outputs, con_loss) if con_loss is not None else (loss, outputs)
        return (loss, con_loss) if con_loss is not None else loss
