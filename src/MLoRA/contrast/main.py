...
def main(parser):
    ...
    if model_args.model_name_or_path.split("/")[-1] in ("chatglm-6b", "chatglm3-6b"):
    ...
    def compute_metrics(eval_preds):
        ...
        macro_f1 = f1_score(decoded_labels, decoded_preds, labels=all_labels, average="macro", zero_division=0)
    ...
    if model_args.use_achievement_loss and task_flag:
        ...
        logger.info(f"Achievement loss enabled: gamma={model_args.achievement_gamma}, "
                     f"margin={model_args.achievement_margin}, task_num={model_args.task_num}, "
                     f"targets={targets_str or 'default(1.0)'}")
