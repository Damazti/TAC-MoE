...
class MMOELoraLinear(nn.Linear, MMOELoraLayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.disable_adapters:   # No adapter
            ...
            # expert_weight shape: (batch_size, expert_num)
            # x shape: (seq, batch, hidden) for ChatGLM3 or (batch, seq, hidden) for Qwen2
            ew_batch_dim = expert_weight.shape[0]  # batch_size
            if x.shape[0] == ew_batch_dim:
                # batch-first layout (batch, seq, hidden) — e.g. Qwen2
                ew_reshape = lambda ew: ew.unsqueeze(1).unsqueeze(2)  # (batch, 1, 1)
            else:
                # seq-first layout (seq, batch, hidden) — e.g. ChatGLM3
                ew_reshape = lambda ew: ew.unsqueeze(-1).unsqueeze(0)  # (1, batch, 1)
